...
<body class="bg-white text-black">
    ...
    <!-- Cover photo and para -->
    <div class="mt-2 flex flex-col lg:flex-row bg-gray-200 h-auto lg:h-[350px]">
        <div class="lg:w-1/2 w-full">
            <img src="{{ asset('images/fragrances-cover-photo.jpg') }}" alt="Fragrances Cover" class="w-full h-full object-cover">
        </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                @if ($mens->count() > 0)
                    @foreach ($mens as $product)
                        <a href="{{ route('product', ['name' => urlencode($product->Name)]) }}" class="block">
                            <div class="bg-white p-4 text-center flex flex-col items-center">
                                <img src="{{ $product->ImageUrl }}" alt="{{ $product->Name }}" class="h-40 w-40 object-cover mb-4 rounded">
                                <p class="text-gray-800 font-medium">{{ $product->Name }}</p>
                                <p class="text-gray-600">LKR {{ $product->Price }}</p>
                            </div>
                        </a>
                    @endforeach
                @else
